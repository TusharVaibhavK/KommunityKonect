...
from service_agents.serviceman_agent import run_agent  # Agent integration
    ...
    try:
        ...
        if command.startswith("/jobs complete ") or command.startswith("complete "):
            ...
            if completion_success:
                send_message(chat_id, f"✅ Job #{job_id[-6:]} marked as complete. User has been notified.")
            else:
                send_message(chat_id, f"⚠️ Failed to update job or notify user. Please try again.")
            return
        ...
        if command.startswith("/assign ") or command.startswith("assign "):
            ...
            if assignment_success:
                send_message(chat_id, f"✅ Job #{job_id[-6:]} assigned to you. User has been notified.")
            else:
                send_message(chat_id, f"⚠️ Failed to update job or notify user. Please try again.")
            return
...
def start_request_flow(chat_id):
    ...
    categories = ["Plumbing", "Electrical", "Carpentry", "Appliance Repair", "Painting", "Other"]
...
def handle_category_selection(chat_id, category):
    ...
    if chat_id not in USER_STATES:
        USER_STATES[chat_id] = {}
    USER_STATES[chat_id] = {"state": "AWAITING_DESCRIPTION", "category": category}
...
def handle_photo(chat_id, photo_file_id=None):
    ...
    if photo_file_id:
        ...
        if file_path_response["ok"]:
            ...
            # Optional: Analyze photo with AI
            try:
                analysis = analyze_telegram_photo(photo_url)
                request_data["photo_analysis"] = analysis
            except Exception as e:
                logging.error(f"Photo analysis error: {str(e)}")
...
def process_message(chat_id, text, message):
    ...
    state = user_state.get("state") if isinstance(user_state, dict) else user_state
...
def handle_updates():
    ...
    try:
        updates = requests.get(
            ...
            params={"offset": LAST_UPDATE_ID + 1, "timeout": 30},  # Use long polling
            timeout=35
        ).json()
...
def show_current_requests(chat_id):
    ...
    if active_requests:
        ...
        for req in active_requests:
            ...
            if req.get("assigned_to"):
                serviceman = users_col.find_one({"username": req["assigned_to"]})
                if serviceman:
    ...
    if completed_requests:
        ...
        if len(completed_requests) > 3:
            send_message(chat_id, f"_...and {len(completed_requests) - 3} more completed requests_")
...
if __name__ == "__main__":
    ...
    logging.info("Bot started with deduplication enabled")
    while True:
        handle_updates()
        time.sleep(0.5)